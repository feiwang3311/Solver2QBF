...
class NeuroSAT0(object):
    ...
    def reference_one(self, problem):
        d = self.build_feed_dict(problem)
        logits = self.sess.run([self.logits], feed_dict=d)
        # argmax returns 0 if the first cell is larger, which means picking True
        # argmax returns 1 if the second cell is larger, which means picking False
        # if we want assignment, we just do argmin, which returns 1 if picking True, and returns 0 if picking False
        assignment = np.argmin(np.array(logits), axis = -1)
        # we want to return it as a tuple
        return tuple(np.reshape(assignment, [-1]))
